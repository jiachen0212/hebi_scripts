...
def split_image(res_js_path, json_path, path_image, split_target,out_dir=None, roi=None):
    ...
    js_path = os.path.join(json_path, '{}.json'.format(pre_name))
    if not os.path.exists(js_path):
        return
    anno_template = None
    if os.path.isfile(js_path):
        with open(js_path, "r", encoding="utf-8") as reader:
    ...
    for i in range(target_w):
        for j in range(target_h):
            ...
            cropped_json_name = '{}/{}_{}_{}_.json'.format(res_js_path, os.path.basename(js_path).split('.')[0], i, j)
            # print(cropped_img_name)
            # print(cropped_json_name)
            # print('\n')cd
...
def split_img_dir(source_path,json_path,split_target,out_dir=None,js_out_dir=None,roi=None):
    for root,dirs,files in os.walk(source_path):
        for filename in files:
            ...
            if ext in ('.jpg', '.jpeg', '.png', '.ppm', '.bmp', '.pgm', '.tif', '.tiff', '.webp'):
                ...
                split_image(js_out_dir, json_path,os.path.join(root,filename),split_target,tmp_dir_out,roi)


if __name__ == "__main__":
    '''
        Vimo seg默认输入图像大小为图高度1024、宽度768
        在下方设置裁剪份数，使得裁剪之后的图像尽可能接近上述尺寸；裁剪的份数必须是整数
    '''

    # 裁剪单张图像
    # path_image = '/Users/chenjia/Desktop/2/2.png'
    # split_target = (2,1)
    # split_image(path_image, split_target,roi=roi)

    # 切割左右两份
    source_path = '/data/home/jiachen/data/seg_data/hebi/20220308/imgs'
    json_dir = '/data/home/jiachen/data/seg_data/hebi/20220308/jsons'
    dir1 = '/data/home/jiachen/data/seg_data/hebi/mask_yayin_2bins'
    dir2 = '/data/home/jiachen/data/seg_data/hebi/json_yayin'
    img_out_dir = '/data/home/jiachen/data/seg_data/hebi/mask_yayin_2bins1'
    js_out_dir = '/data/home/jiachen/data/seg_data/hebi/json_yayin1'
    split_target = (2, 1)
    split_img_dir(source_path,json_dir, split_target,img_out_dir, js_out_dir)

    # single_bins, 再走json+img切割. [切分2x4份]
    # roi = (300, 0, 3896, 7592)
    # out_dir = '/data/home/jiachen/data/seg_data/hebi/seg_yayin'
    # split_target = (2, 4)
    # split_img_dir(source_path,json_dir, split_target,out_dir,roi)

    txt_0308 = open(r'./0308.txt', 'w')
    ims = os.listdir(img_out_dir)
    for im in ims:
        im_path_, js_path = os.path.join(dir1, im)[5:], os.path.join(js_out_dir, im[:-3]+'json')
        js_path1 = os.path.join(dir2, im[:-3]+'json')
        if os.path.exists(js_out_dir):
            line = "{}||{}\n".format(im_path_, js_path1[5:])
            print(line)
            txt_0308.write(line)
