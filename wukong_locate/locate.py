...
def main():
    ...
    if sig.lower().strip() == "train":
        ...
        img = cv2.imread(os.path.join(train_dir, "image.png"), 0)
        five_holes_area = cv2.imread(os.path.join(train_dir, "5holes.png"), 0)
        roi = cv2.imread(os.path.join(train_dir, "template.png"), 0)
    ...
    if sig.lower().strip() == "inference":
        ...
        with open(inference_list) as f:
            for path in f:
                ...
                if draw is not None:
                    draw_file = ".".join(path.split(".")[:-1]) + "_draw.png"
